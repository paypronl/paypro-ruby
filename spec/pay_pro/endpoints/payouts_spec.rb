...
RSpec.describe PayPro::Endpoints::Payouts do
  describe '#list' do
    subject(:list) { endpoint.list }

    let(:endpoint) { described_class.new(api_client: default_api_client) }

    let(:url) { 'https://api.paypro.nl/payouts' }

    before do
      stub_request(:get, url).and_return(
        body: File.read('spec/fixtures/payouts/list.json')
      )
    end
    ...
    it 'has payouts' do
      list
      expect(list.data[0]).to be_a(PayPro::Payout)
    end

    context 'with options' do
      subject(:list) { endpoint.list(api_url: 'https://api-test.paypro.nl') }

      let(:url) { 'https://api-test.paypro.nl/payouts' }
  ...
  describe '#get' do
    ...
    let(:id) { 'POKHCDCQPS4GAA' }
    let(:url) { "https://api.paypro.nl/payouts/#{id}" }

    before do
      stub_request(:get, url).and_return(
        body: File.read('spec/fixtures/payouts/get.json')
      )
    end
    ...
    it 'returns a Payout' do
      expect(get).to be_a(PayPro::Payout)
    end
    ...
    context 'with options' do
      ...
      let(:url) { "https://api-test.paypro.nl/payouts/#{id}" }
  ...
  describe '#create' do
    ...
    it 'returns a Mandate' do
      expect(create).to be_a(PayPro::Payout)
    end
